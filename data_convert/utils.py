...
def get_schema(event):
    ...
    return set([(event_type, argument[0], argument[1]) for argument in event['arguments']])
...
def output_schema(event_schema_set, output_file):
    ...
    entity_type_list = list(set([schema[2] for schema in event_schema_set]))
    ...
    for event_type, arg_role, entity_type in event_schema_set:
        if arg_role is None:
            continue
    ...
    with open(output_file, 'w') as output:
        ...
        output.write(json.dumps(entity_type_list) + '\n')
